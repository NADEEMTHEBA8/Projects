...
export const Model = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('WebGL Renderer initialization failed in Model component:', e);
      return;
    }
  ...
  const blurShadow = useCallback(amount => {
    if (!renderer.current || !blurPlane.current || !renderTarget.current || !renderTargetBlur.current) return;
  ...
  const renderFrame = useCallback(() => {
    if (!renderer.current || !scene.current || !camera.current || !renderTarget.current) return;
    const blurAmount = 5;
...
const Device = ({
  ...
  useEffect(() => {
    const applyScreenTexture = async (texture, node) => {
      if (!renderer.current || !node || !node.material) return;
      texture.colorSpace = SRGBColorSpace;
      texture.flipY = false;
      texture.anisotropy = renderer.current.capabilities ? renderer.current.capabilities.getMaxAnisotropy() : 1;
      texture.generateMipmaps = false;

      // Decode the texture to prevent jank on first render
      if (renderer.current.initTexture) {
